...
drawstatus(Monitor* m) {
  int w = 0;
    ...
    drw_setscheme(drw, scheme[SchemeNorm]);
		...
		int tw = TEXTW(stext) - lrpad + 2; /* 2px right padding */
    int x = m->ww - tw - (ulinepad * 2) - (sp * 2);

    // TODO: multiple blocks, directly inside here no extra program (slstatus/dwmblocks) needed
		drw_text(drw, x, 0, tw + ulinepad, bh, ulinepad, stext, 0); 
    drw_rect_col(drw, x, bh-ulinestroke-ulinevoffset, tw + (ulinepad * 2), ulinestroke, 1, scheme[SchemeBar][ColFg].pixel);
